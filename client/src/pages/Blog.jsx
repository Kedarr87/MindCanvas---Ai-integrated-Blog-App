...
const Blog = () => {
  ...
  const getBlogs = async(id) => {
    try {
      const res = await axios.get(`${process.env.URL}/api/blog/${id}`)
  ...
  const fetchComments = async() => {
    const res = await axios.get(`${process.env.URL}/api/blog/${id}/comments`)
  ...
  const addComment = async(e) => {
    ...
    try {
      const res = await axios.post(`${process.env.URL}/api/blog/${id}/add-comment`, {name, content})
  ...
  return data ? (
    <div className='relative'>
      ...
      <div className='text-center mt-20 text-gray-600'>
          ...
          <p className='inline-block py-1 px-4 rounded-full mb-6 border text-sm border-primary/35 bg-primary/5 font-medium text-primary'>Kedar Patil</p>
      </div>
